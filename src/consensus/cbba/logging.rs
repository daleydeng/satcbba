...
use crate::consensus::types::{Agent, Task, TaskId, BidInfo, AgentId};
...
pub fn log_dropped_tasks<T, A, C>(
    cbba: &CBBA<T, A, C>,
    dropped: &HashMap<TaskId, Vec<TaskId>>
)
where
    T: Task,
    A: Agent + Display,
...
{
    if !dropped.is_empty() {
        for (task_id, successors) in dropped {
            if !successors.is_empty() {
                cbba_info!("  {} released task {} and its successors: {:?}", cbba.agent, task_id, successors);
            } else {
...
pub fn log_iteration_status<T, A, C>(
    cbbas: &[CBBA<T, A, C>],
    iteration: usize
)
where
    ...
    C: ScoreFunction<T, A>,
{
    ...
    for cbba in cbbas {
        ...
        let total_score: f64 = cbba.bids.values().map(|info| match info {
            BidInfo::Winner(_, s, _) => s.val() as f64,
            BidInfo::None(_) => 0.0,
        }).sum();

        let bundle_str = bundle.iter().map(|t| format!("{}", t)).collect::<Vec<_>>().join(", ");
        let path_str = path.iter().map(|t| format!("{}", t)).collect::<Vec<_>>().join(", ");
    ...
    C: ScoreFunction<T, A>,
{
    ...
    for cbba in cbbas {
        ...
        if !bundle_tasks.is_empty() {
            let bundle_str = bundle_tasks.iter().map(|t| format!("{}", t)).collect::<Vec<_>>().join(", ");
            let path_str = path_tasks.iter().map(|t| format!("{}", t)).collect::<Vec<_>>().join(", ");
            cbba_info!("{} bundle: [{}]", agent, bundle_str);
            cbba_info!("{} path: [{}]", agent, path_str);
        } else {
...
pub fn log_assignment_table<T, A, C>(
    cbbas: &[CBBA<T, A, C>],
    tasks: &[T]
)
where
    ...
    C: ScoreFunction<T, A>,
{
    ...
    let mut task_assignments: HashMap<TaskId, Option<AgentId>> = tasks.iter().map(|t| (t.id(), None)).collect();
